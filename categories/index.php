<?php 
$config_file = '../templates/core/config.php';
$config = include($config_file);

$page = isset($_GET['page']) ? intval($_GET['page']) : 1;
$query = isset($_GET['s']) ? urlencode($_GET['s']) : ''; 
$api_url = "https://vidwish.com/api/vidwish.php?page={$page}&s={$query}&limit=12&content=0&order=random";
$response = file_get_contents($api_url);
$data = json_decode($response, true);

if (!$data || !isset($data['videos'])) {
    echo "Gagal mengambil data dari API.";
    exit;
}

$videos = $data['videos'];
$current_page = $data['current_page'];
$total_pages = $data['total_pages'];


$api_url_genre = "https://vidwish.com/api/vidwish.php?content=2&limit=1000";
$response_genre = file_get_contents($api_url_genre);
$data_genre = json_decode($response_genre, true);

$genreList = [];

if ($data_genre && isset($data_genre['videos'])) {
    foreach ($data_genre['videos'] as $video) {
        if (!empty($video['genre'])) {
            $genres = explode(',', $video['genre']);
            foreach ($genres as $genre) {
                $clean_genre = trim($genre);
                if ($clean_genre !== '') {
                    $genreList[] = $clean_genre;
                }
            }
        }
    }
}


// Hapus duplikat
$uniqueGenres = array_unique($genreList);
sort($uniqueGenres); // Optional: urutkan
?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta name="description" content="<?= htmlspecialchars($config['site_description']) ?>">
    <meta name="keywords" content="<?= htmlspecialchars($config['site_keywords']) ?>">
    <meta name="author" content="Vidwish">
    <title>Categories : <?= htmlspecialchars($_GET['s']) ?></title>

    <link rel="icon" href="/templates/assets/favicon.ico" type="image/x-icon" />
    <link rel="stylesheet" href="https://unicons.iconscout.com/release/v4.0.8/css/line.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <?php include($_SERVER['DOCUMENT_ROOT'] . '/templates/assets/styles.php'); ?>
    
    <style>
      #categories-videos, #categories-all{
      	display: none;
      } 
      .category-button {
        display: inline-block;
        margin: 5px;
        padding: 10px 15px;
        background-color: var(--red-color);
        border-radius: 5px;
        text-decoration: none;
        color: #FFF;
        transition: background-color 0.2s;
        font-size: 14px;
        max-width: 100%;
        word-wrap: break-word;
        filter: brightness(0.9);
      }
      .category-button:hover {
        background-color: var(--light-gray-color);
      }
	  .category-all {
          margin-top: 30px;
      }
      @media (max-width: 600px) {
        .category-button {
          font-size: 12px; /* Ukuran font lebih kecil pada perangkat kecil */
          padding: 5px 10px; /* Mengurangi padding pada layar kecil */
        }
      }
    </style>
  </head>
  <body class="dark-mode">
    <div class="container">
      <?php include($_SERVER['DOCUMENT_ROOT'] . '/templates/parts/header.php'); ?>
      <main class="main-layout">
        <div class="screen-overlay"></div>
        <?php include($_SERVER['DOCUMENT_ROOT'] . '/templates/parts/sidebar.php'); ?>
        <div class="content-wrapper">
          <?php include($_SERVER['DOCUMENT_ROOT'] . '/templates/parts/categories.php'); ?>
          
          <div id="categories-videos">
            <!-- Search Results Parameter -->
            <div class="title-wrap">
              <h2 class="section-title top">Search Categories for: "<?= htmlspecialchars($_GET['s']) ?>"</h2>
            </div>
            <div class="video-list">
              <?php foreach ($videos as $video): ?>
              <a href="/watch/?v=<?= htmlspecialchars($video['slug']) ?>" class="video-card">
                <div class="thumbnail-container">
                  <img src="<?= htmlspecialchars($video['poster']) ?>" alt="Video Thumbnail" class="thumbnail" />
                  <div class="thumbnail-overlay">
                    <i class="fa fa-play play-icon"></i>
                  </div>
                  <?php if (strpos($video['title'], 'Bokep Indo') === 0): ?>
                  <p class="duration">
                    <?= (function($time) {
                        $parts = explode(':', htmlspecialchars($time));
                        return count($parts) === 3 && $parts[0] === '00' ? $parts[1] . ':' . $parts[2] : $time;
                    })($video['time']) ?>
                  </p>
                  <?php else: ?>
                  <?php endif; ?>
                </div>
                <div class="video-info">
                  <div class="video-details">
                    <h2 class="title"><?= htmlspecialchars($video['title']) ?></h2>
                    <p class="views"><i class="uil uil-eye"></i><?= htmlspecialchars($video['views']) ?> <i class="uil uil-calendar-alt"></i><?= date("d M Y", strtotime($video['date'])) ?></p>
                  </div>
                </div>
              </a>
              <?php endforeach; ?>
            </div>

            <!-- Pagination -->
            <div class="pagination">
              <a href="/categories?s=<?= htmlspecialchars($_GET['s']) ?>&page=1" class="page-link <?= $current_page === 1 ? 'active' : '' ?>">1</a>
              <?php if ($current_page > 4): ?>
              <span class="page-link">...</span>
              <?php endif; ?>
              <?php
              $start = max(2, $current_page - 1);
              $end = min($total_pages - 1, $current_page + 1);
              for ($i = $start; $i <= $end; $i++): 
              ?>
              <a href="/categories?s=<?= htmlspecialchars($_GET['s']) ?>&page=<?= $i ?>" class="page-link <?= $i === $current_page ? 'active' : '' ?>"><?= $i ?></a>
              <?php endfor; ?>
              <?php if ($current_page < $total_pages - 3): ?>
              <span class="page-link">...</span>
              <?php endif; ?>
              <?php if ($total_pages > 1): ?>
              <a href="/categories?s=<?= htmlspecialchars($_GET['s']) ?>&page=<?= $total_pages ?>" class="page-link <?= $current_page === $total_pages ? 'active' : '' ?>"><?= $total_pages ?></a>
              <?php endif; ?>
            </div><br><br>
          </div>
          
          <div id="categories-all">
            <!-- Search Results No Parameter -->
            <div class="title-wrap">
              <h2 class="section-title top">List Categories</h2>
            </div>
            <div class="category-all">
              <?php foreach ($uniqueGenres as $genre): ?>
                <a href="/categories?s=<?= urlencode($genre) ?>" class="category-button" id="<?= htmlspecialchars($genre) ?>">
                  <?= htmlspecialchars($genre) ?>
                </a>
              <?php endforeach; ?>
            </div><br><br><br><br>
          </div>
        </div>
      </main>
    </div>
    <script>
      function addActiveClass() {
        const urlParams = new URLSearchParams(window.location.search);
        const s = urlParams.get('s');

        let activeButtonId = s ? s : 'Lainnya';  // Jika s ada, gunakan s, jika tidak gunakan Selengkapnya

        const ActiveButton = document.getElementById(activeButtonId);
        if (ActiveButton) {
          ActiveButton.classList.add('active');
        }
      }

      document.addEventListener('DOMContentLoaded', function() {
        addActiveClass();
      });
    </script>
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        const urlParams = new URLSearchParams(window.location.search);
        const s = urlParams.get('s');

        const categoriesSection = document.getElementById("categories-videos");
        const categoriesList = document.getElementById("categories-all");

        if (!s) {
          if (categoriesList) {
            categoriesList.style.display = "block";
            document.title = "List Categories - <?= htmlspecialchars($config['site_title']) ?>";
          }
        } else {
          if (categoriesSection) {
            categoriesSection.style.display = "block";
          }
        }
      });
    </script>

  	<?php include($_SERVER['DOCUMENT_ROOT'] . '/templates/assets/javascript.php'); ?>
  </body>
</html>
